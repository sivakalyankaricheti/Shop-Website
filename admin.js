...
$('#bankForm').addEventListener('submit', event => { event.preventDefault(); localStorage.setItem(BANK_KEY, JSON.stringify({accountName: $('#accountName').value.trim(), bankName: $('#bankName').value.trim(), sortCode: $('#sortCode').value.trim(), accountNumber: $('#accountNumber').value.trim(), paymentNote: $('#paymentNote').value.trim()})); $('#bankStatus').textContent = 'Owner bank details saved.'; });
